...
const CompletedAppointments = () => {
  ...
  const fetchAppointments = async () => {
    try {
      ...
      const completedOnly = appointmentsWithPayments.filter(
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-green-50/20 to-gray-50 pt-24 pb-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-gradient-to-r from-green-500 to-green-600 rounded-2xl shadow-xl p-8 mb-8">
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="text-3xl md:text-4xl font-bold text-white">Completed Appointments</h1>
              <p className="text-green-100 mt-1">{appointments.length} completed appointments</p>
            </div>
          </div>
        </div>
            ...
            {appointments.length === 0 ? (
              ...
              <div className="space-y-4">
                {appointments.map((appointment) => (
                  <div
                    key={appointment._id}
                    className="bg-gradient-to-r from-green-50 to-white rounded-xl p-6 border-2 border-green-200 shadow-md hover:shadow-lg transition-shadow"
                  >
                    ...
                    <div className="mb-4">
                      ...
                      {appointment.paymentStatus === "paid" ? (
                        <div className="flex items-center text-green-700 bg-green-50 rounded-lg px-3 py-2">
                          <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                          </svg>
                          <span className="font-medium">Paid €{appointment.amount}</span>
                          <span className="ml-2 text-sm">({appointment.customerType})</span>
                        </div>
                      ) : (
                        <button
                          onClick={() => handlePayment(appointment)}
                          className="flex items-center text-blue-600 hover:text-blue-700 bg-blue-50 hover:bg-blue-100 rounded-lg px-3 py-2 transition-colors font-medium"
                        >
                          <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div className="flex pt-4 border-t border-green-200">
                      <button
